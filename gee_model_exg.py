...
WINDOW = 6                                # t-5…t -> t+1
SEED = 42
# Preferred boundary; script will fall back to global 80/20 if this yields too little train data
...
def fetch_weekly_abs_humidity(weeks_list, geom: ee.Geometry):
    ...
    for wk in weeks_list:
        ...
        img   = col.filterDate(start, end).mean()
        stats = img.reduceRegion(ee.Reducer.mean(), geom, scale=10000, maxPixels=1e13)
...
def _resolve_pm25_source(sample_date_iso: str):
    """Probe candidate CAMS datasets/bands to find an available PM2.5 source."""
    candidates = [
        ("ECMWF/CAMS/NRT",        ["particulate_matter_d_less_than_2p5um_surface", "pm2p5", "pm2p5_mass"]),
        ("ECMWF/CAMS/REANALYSIS", ["particulate_matter_d_less_than_2p5um_surface", "pm2p5"])
    ]
    for ds, bands in candidates:
        try:
...
def fetch_weekly_pm25(weeks_list, geom: ee.Geometry):
    """Weekly mean surface PM2.5 (µg/m^3) using ECMWF CAMS if available; otherwise return all-NaN series."""
    ...
    if ds is None:
        print("[WARN] Could not resolve a CAMS PM2.5 source. PM2.5 will be NaN and the feature will be auto-disabled.")
    ...
    for wk in weeks_list:
        ...
        img = col.mean()
        try:
            val = img.reduceRegion(ee.Reducer.mean(), geom, scale=20000, maxPixels=1e13).get(band).getInfo()
        except Exception:
            val = None
        rows.append({"week_start_date": pd.Timestamp(wk), "pm25_ugm3": val})
    return pd.DataFrame(rows)
...
# ---- Per-ZIP validation with fallback already covered by global split ----
