...
analysis_start = pd.to_datetime('2019-09-30')  # Start of analysis period
analysis_end   = pd.to_datetime('2022-12-31')    # End of analysis period

# Load the CSV file with columns: week_start, week_end, and percent.
file_path = 'zip_code/60601.csv'  # Update with your CSV file path.
...
def fetch_surface_temperature(start_date, end_date):
    ...
    point = ee.Geometry.Point([-87.622844, 41.886262])
...
ax2 = ax1.twinx()
color_percent = 'blue'
ax2.set_ylabel("Incident Activity Level", color=color_percent)
ax2.plot(data['week_start'], data['ILI_Activity_Level'], marker='x', color=color_percent, label='Incident %')
