...
def compute_ccf(df, temperature_data, max_lag=10):
    ...
    # Check if standard deviation is 0 or null
    if df['ILI_Activity_Level'].std() == 0 or df['Temperature'].std() == 0 or df['ILI_Activity_Level'].std() is None or df['Temperature'].std() is None:
        return "NA", []
...
def compute_covariance_lag(df, temperature_data, max_lag=10):
    ...
    covariance_values = [df['ILI_Activity_Level'].cov(df['Temperature'].shift(lag)) for lag in range(-max_lag, max_lag + 1)]
    ...
    for file_name in os.listdir(directory_path):
        if file_name.endswith('.csv'):
            ...
            for (start_date, end_date), period_label in zip(periods, period_labels):
                ...
                best_lag, _ = compute_covariance_lag(df, temperature_data)
